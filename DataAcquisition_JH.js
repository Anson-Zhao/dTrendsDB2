...
function axiosReq_JH() {
    ...
    con.connect(function (err) {
        if (err) {
            ...
            console.log("MySql Connected");
            //download data from OGC
            axios.get('https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/08-04-2020.csv')
                .then((response) => {
                    ...
                    let data_Json = CSVToArray(response);
...
// function dataProcessing_JH(download) {
//     let countryN;
//
//     for (let i = 0; i < download.data.features.length; i++) {
//         // acquire data from OGC, make sure the data format is correct
