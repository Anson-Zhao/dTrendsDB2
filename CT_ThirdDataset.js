...
let Date_n =  "2020-12-20"
let First_Data ="SELECT * FROM dtrends.covid_19_OGC WHERE Date = ?;"
let Sec_Data = "SELECT * FROM dtrends.covid_19_JH WHERE Date = ?;"
...
con.query(Sec_Data,Date_n,function (err, Sec_Data) {
    ...
    con.query(First_Data,Date_n,function (err, First_Data) {
        ...
        //     let Only_Data = "SELECT LayerName FROM dtrends.covid_19_OGC WHERE LayerName = ?;"
        ...
        //             });
        //         }else{
        //             let Third_data_a = "INSERT INTO dtrends.covid_19_Final(Date, LayerName, LayerType, FirstLayer, DisplayName, CaseNum, DeathNum, RecovNum," +
        //                 " ActiveNum, Latitude, Longitude,CityName, StateName, CountryName, ContinentName, Color_Confirmed, Color_Death, Color_Recovered) " +
        //                 "VALUES (?,?,'H_PKLayer','Corona_Virus',?,?,?,?,?,?,?,?,?,?,?,'rgb(220,0,0) rgb(220,0,0) rgb(220,0,0)','rgb(0,0,0) rgb(0,0,0) rgb(0,0,0)','rgb(124,252,0) rgb(124,252,0) rgb(124,252,0)'); ";
        ...
        for(let k = 0; k < First_Data.length; k++){
            let Only_Data_a = "SELECT LayerName FROM dtrends.covid_19_JH WHERE LayerName = ?;"
            con.query(Only_Data_a, First_Data[k].LayerName, function (err, result) {
                if(result == ""){
                    let Third_data_OGC = "INSERT INTO dtrends.covid_19_Final(Date, LayerName, LayerType, FirstLayer, DisplayName, CaseNum, DeathNum, RecovNum," +
                        " ActiveNum, Latitude, Longitude,CityName, StateName, CountryName, ContinentName, Color_Confirmed, Color_Death, Color_Recovered) " +
                        "VALUES (?,?,'H_PKLayer','Corona_Virus',?,?,?,?,?,?,?,?,?,?,?,'rgb(220,0,0) rgb(220,0,0) rgb(220,0,0)','rgb(0,0,0) rgb(0,0,0) rgb(0,0,0)','rgb(124,252,0) rgb(124,252,0) rgb(124,252,0)'); ";
